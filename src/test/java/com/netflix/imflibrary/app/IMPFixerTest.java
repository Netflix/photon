...
public class IMPFixerTest {
    ...
    @Test(expectedExceptions = IMFException.class, expectedExceptionsMessageRegExp = ".*WaveAudioEssenceDescriptor.*is missing one/all of MCATitle.*")
    public void testMainErrorCoverage() throws Exception {
        IMPFixer.main(new String[] {
                TestHelper.findResourceByPath("TestIMP/1-sec-imp").getAbsolutePath(),
                Files.createTempDirectory(null).toFile().getAbsolutePath()
        });
    }
