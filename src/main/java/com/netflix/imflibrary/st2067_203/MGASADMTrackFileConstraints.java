            ...
            if (sequence == null) {
                ...
                if (genericDescriptor instanceof MGASoundEssenceDescriptor) { // Support for st2067-203
                    ...
                    if (subDescriptors.size() == 0) {
                        ...
                        if (mgaAudioMetadataSubDescriptors.size() == 0) {
                            imfErrorLogger.addError(IMFErrorLogger.IMFErrors.ErrorCodes.IMF_ESSENCE_COMPONENT_ERROR, IMFErrorLogger.IMFErrors.ErrorLevels.FATAL, IMF_MGASADM_EXCEPTION_PREFIX +
                                    String.format("MGASoundEssenceDescriptor in the IMFTrackFile represented by ID %s does not reference a MGAAudioMetadataSubDescriptor", packageID.toString(), mgaAudioMetadataSubDescriptors.size()));
                        } else {
                            // ST 2067-203 section 5.6.2 Table 3
